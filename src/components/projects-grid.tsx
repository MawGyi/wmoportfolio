        ...
        {projects.map((project) => (
          <div key={project._id} className="group relative border-b border-border pb-8 last:border-0 last:pb-0">
             <div className="flex flex-col sm:flex-row justify-between sm:items-baseline gap-2 mb-3">
                <h3 className="text-title group-hover:text-accent transition-colors">
            ...
            {project.description && (
              <p className="text-caption text-muted-foreground mb-4 max-w-xl">
                {project.description}
              </p>
            )}

            {project.tags && project.tags.length > 0 && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-micro text-muted-foreground font-mono">
